...
export class AiChatService {
  ...
  public enviar(prompt: string): void{
    ...
    const userMsg: IChatMessage = { messageType: 'USER', content: prompt };
    ...
    this.http.post(
      ...
      .subscribe({
        next: (resposta) => {
          const aiMsg: IChatMessage = { messageType: 'ASSISTANT', content: resposta };
          this.menssages.update(list => [...list, aiMsg]);
        },
        error: (err) => {
          console.error('Erro no envio:', err);
          const errorMsg: IChatMessage = { messageType: 'ASSISTANT', content: '⚠️ Erro ao processar requisição.' };
  ...
  public carregarHistorico(): void {
    this.http.get<IChatMessage[]>(
      `${this.urlBase}/history`,
      {
        params: {
